...
def _fake_library(tmp_path: Path) -> tuple[Path, Path, str, str]:
    ...
    catalog = fill_valid_catalog_v31(
        ...
        title_zh="内容消费者测试",
        title_original="Content Only Consumer Test",
        domain="consumer_test",
    )
    catalog["library_locator"]["paper_id"] = paper_id
    ...
    write_asset_manifest(folder, prefix=paper_id, paper_number=paper_number, paper_id=paper_id, stage="papers")

    all_entry = {
        "paper_number": paper_number,
        "paper_id": paper_id,
        "paper_dir": str(folder),
        "asset_refs": {"markdown": str(md_path), "pdf": str(pdf_path), "images_dir": str(images)},
    ...
    _write_json(all_catalog, {"schema_version": "3.1", "updated_at": "", "papers": [all_entry]})
