...
paper_name = "2024_wang_test_paper"
PAPER_NUMBER = "0000000000000001"


class FakeCatalog:
    def list_papers(self):
        return [{"paper_name": paper_name, "paper_number": PAPER_NUMBER}]
...
def _job(tmp_path):
    ...
    (jdir / "planning" / "selected_papers.json").write_text(json.dumps({
        "selected_papers": [{"paper_name": paper_name, "paper_number": PAPER_NUMBER}],
        "selection_status": "confirmed",
    }, ensure_ascii=False), encoding="utf-8")
    article = jdir / "article" / PAPER_NUMBER
    article.mkdir(parents=True)
    (article / f"{paper_name}.md").write_text("# Full text\n\nfull text", encoding="utf-8")
    (article / f"{paper_name}.metadata.json").write_text(
    ...
    (jdir / "planning" / "workset_manifest.json").write_text(json.dumps({
        "job_id": info["job_id"],
        "copied": [{"paper_name": paper_name, "paper_number": PAPER_NUMBER, "work_dir": str(article)}],
        "skipped": [],
...
def test_deep_read_refuses_user_filled_note(tmp_path):
    ...
    note = jdir / "reading" / "paper_notes" / f"{paper_name}.md"
