...
def test_network_style_frozen_catalog_formalize_commit(tmp_path: Path):
    ...
    final=papers/result["paper_id"]; assert result["phase"]=="complete" and final.is_dir() and not workspace.root.exists(); assert compute_sha256(final/f"{result['paper_id']}.metadata.json")==metadata_hash; assert compute_sha256(final/f"{result['paper_id']}.catalog.json")==catalog_hash
