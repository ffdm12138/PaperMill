...
def _workspace(tmp_path: Path)->tuple[PaperRawWorkspace,Path,Path,Path]:
    ...
    metadata=_metadata(folder); atomic_write_json(folder/f"{NUMBER}.metadata.json",metadata,indent=2)
    # A real PDF with the title, a labeled DOI, and an author line in the
    # decoded text layer (the v2 extractor needs the bibliographic fields
    # for first-page DOIs to reach strong evidence).
    pdf_doc = fitz.open(); pdf_page = pdf_doc.new_page(); pdf_page.insert_text((72, 72), "Terrain Wind Downscaling"); pdf_page.insert_text((72, 100), "doi:10.1234/example2024"); pdf_page.insert_text((72, 114), "Jane Smith"); pdf_doc.save(str(folder / f"{NUMBER}.pdf"), deflate=True); pdf_doc.close()
    ...
    evidence=extract_pdf_identity_evidence(pdf_path=folder/f"{NUMBER}.pdf"); write_match_receipt(folder,build_match_receipt(folder,NUMBER,metadata,evidence,requested_doi="10.1234/example2024",provider_records=[metadata["source"]["raw_record_path"]])); freeze_metadata(folder,NUMBER); assert_metadata_frozen(folder,NUMBER)
    ...
    write_import_status(folder,"staged_metadata",extra={"paper_number":NUMBER,"paper_raw_id":NUMBER,"source_type":"network_search","source_provider":"fixture","doi":"10.1234/example2024"})
    # Write a discovery receipt to satisfy lifecycle inspection.
    write_or_validate_discovery_receipt(folder/f"{NUMBER}.discovery_receipt.json",build_receipt_payload(paper_number=NUMBER,candidate_id="test",page_id="test",keyword_id="test",provider="fixture",normalized_doi="10.1234/example2024"),workspace_root=raw)
