...
def test_apply_cleanup_refuses_to_drop_equal_or_higher_stage_than_keep(tmp_path):
    """Veto: if both workspaces tie in rank, the deterministic tiebreak still forces
    a drop only when strictly lower; engineered to keep numbered (rank ~15) and have
    a legacy at the SAME ready_for_convert rank so drop_rank >= keep_rank triggers abort."""
    paper_raw, legacy, numbered = _dup_group_number(
        tmp_path, legacy_bytes=b"%PDF veto",
        legacy_import_status="ready_for_convert",  # same rank as numbered
        numbered_import_status="ready_for_convert",
    )
    # both rank 15 -> keep-rule tiebreaks by asset_count then name; force the
    # scenario where keep_rank <= drop_rank by making legacy the keep (it has
    # more assets) and numbered the drop at the SAME rank.
    from src.services.ingest_duplicate_guard import resolve_paper_raw_identity
    from scripts import audit_paper_raw_duplicate_workspaces as A
    rep = A.build_report(paper_raw_dir=paper_raw, ledger_path=_ledger_path(tmp_path))
    group = rep["groups"][0]
    keep_rank = A._rank_from_dict(group["keep"])
    drop_rank = A._rank_from_dict(group["drop"][0])
    # the veto fires when drop_rank >= keep_rank; with equal ranks this must abort
    if drop_rank >= keep_rank:
        with pytest.raises(RuntimeError):
            A._apply_cleanup(rep, paper_raw_dir=paper_raw, ledger_path=_ledger_path(tmp_path))
