...
def validate_v2_library(*, papers_dir: Path = PAPERS_DIR, ledger_path: Path = PAPER_NUMBER_LEDGER_PATH, catalog_root: Path = CATALOG_FOLDER_ROOT, notebook_dir: Path | None = None) -> dict:
    ...
    registry=FormalPaperRegistry(papers_dir=Path(papers_dir),ledger=PaperNumberLedger(ledger_path)); report=doctor(root=Path(catalog_root),formal_registry=registry,notebook_dir=Path(notebook_dir),transaction_root=TRANSACTION_ROOT); valid = report["folder_integrity_safe"] and report["classification_complete"] and report["writer_category_safe"] and not report["errors"]; return {"valid":valid,**report}
