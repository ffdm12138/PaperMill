...
- 关键词 discovery 双通道（Refresh + Backfill）：当前 active path 使用 in-process coordinator。Provider page 先写入 `data/discovery/pending_pages/` durable journal，再用 expected-cursor CAS 推进 Backfill cursor；pending candidate 通过 claim/lease、DOI/resolution lock、sidecar receipt 和 durable export 实现 effectively-once outcome。`--max-workers` 是全局 lane 上限；`max_candidates` 是 drain budget，不会丢弃已抓取候选。v1 notebook cursor 不可信，必须迁移后重扫。runtime notebooks/pending pages/locks/exports/logs/reports 不入 git/snapshot。详见 `docs/PROJECT_CONTRACT.md`「关键词 discovery 双通道契约」与 `AGENTS.md` §9。
- 2026-07-08 discovery audit hardening：Backfill 启动会恢复已 CAS 提交但仍为 `fetched` 的上一页 journal；Backfill notebook generation 使用 expansion-level composite Backfill signature（page size + provider Backfill sort/filter）；title-resolution DOI 会持久化到 journal；network metadata staging 通过 provider raw canonicalizer 逐字段生成 metadata v2.0；discovery receipt/source_record 与 staging 状态在 allocator 写临界区内落盘；DOI duplicate fast path 使用 DOI-only index，最终门禁仍在 staging lock 内执行；page budget exhausted 是正常 stop reason，不计 provider failure；pending backpressure 使用 max/resume 滞回。
