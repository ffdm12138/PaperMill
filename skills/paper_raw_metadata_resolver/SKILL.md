...
description: Resolve citation-ready Metadata from converted PDF evidence. Emit candidates and a pure bibliographic patch; never decide the independent PDF match receipt.
---
...
metadata and does NOT decide `<paper_number>.metadata_match.json`.
Until that independent evidence stage succeeds, the workspace remains metadata-unresolved or metadata-resolved, never implicitly matched.
...
- **不得读取** `data/papers`、`data/catalog/` 或 `write/`。
...
- **不得**写入内嵌 `metadata_match` 或独立 match receipt。resolver 只能
...
- **只负责 metadata candidates 和 metadata patch**；不负责 Catalog 内容，不读写
  分类目录，不从正文内容总结论文贡献，
...
- **权限区分**：LLM-facing skill 和 candidate selection 只确定 citation Metadata；独立 deterministic PDF evidence/match/freeze 阶段才可写 match receipt。identifier conflict 不能被普通人工确认覆盖。
...
- **Case D — md 中完全抽不到候选**：必须联网查。若无法可靠匹配，保持
  不写权威 Metadata，输出 `decision = no_candidates`，等待人工确认，
  不得入库，不得编造 metadata。
...
- 保持 Metadata unresolved（绝不伪造 match receipt）；
