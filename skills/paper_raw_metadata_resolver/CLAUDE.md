...
为 `data/paper_raw/<paper_number>/` 生成带证据的 metadata 候选与只补空字段的 patch。
...
- 必须先读 `data/paper_raw/<paper_number>/<paper_number>.md`（转换后 MinerU Markdown，候选主证据）；
  再读 `<paper_number>.metadata.json`（目标空壳，判断哪些字段为空、哪些需补）。
