...
paper_name 必须通过 validate_paper_name()，防止路径穿越。
"""
import re
from pathlib import Path

# Windows 文件系统非法字符 + paper_name 不允许的字符
# 允许: A-Z a-z 0-9 _ - 和中文 (一-鿿)
_ILLEGAL = re.compile(r"[\\/:*?\"<>|.\s()+\-&#%!;=@~`\[\]{}',‐–—]+")
# 合法 paper_name：字母数字下划线短横线中文
_paper_name_RE = re.compile(r"^[A-Za-z0-9_\-一-鿿]+$")
...
def sanitize_paper_name(raw: str) -> str:
    """清洗字符串为文件系统安全的 paper_name（保留中文，非法字符→_，空白折叠）"""
...
def validate_paper_name(paper_name: str) -> str:
    """校验 paper_name 合法性，防路径穿越。非法抛 ValueError。"""
    if not isinstance(paper_name, str) or not paper_name:
        raise ValueError(f"Invalid paper_name: {paper_name!r}")
    if not _paper_name_RE.match(paper_name):
        raise ValueError(f"Invalid paper_name (含非法字符): {paper_name!r}")
    # 二次防护：禁止分隔符与穿越
    if ".." in paper_name or "/" in paper_name or "\\" in paper_name:
        raise ValueError(f"Invalid paper_name (路径穿越): {paper_name!r}")
    return paper_name
...
def validate_job_id(job_id: str) -> str:
    ...
    suffix 为 6 位 hex。安全规则与 paper_name 一致。
    """
    if not isinstance(job_id, str) or not job_id:
        raise ValueError(f"Invalid job_id: {job_id!r}")
    if not _paper_name_RE.match(job_id):
