...
def build_formalization_plan(workspace: PaperRawWorkspace, *, papers_dir: Path, ledger_path: Path|None=None) -> dict:
    ...
    catalog=json.loads(workspace.catalog.read_text(encoding="utf-8")); paper_id=str(catalog["paper_id"]); final_dir=papers_dir/paper_id
    ...
    if final_dir.exists(): raise FileExistsError(f"paper_id_conflict: {final_dir}")
    source_assets={
    ...
    final_filenames={
        "metadata":f"{paper_id}.metadata.json","catalog":f"{paper_id}.catalog.json",
        "markdown":f"{paper_id}.md","pdf":f"{paper_id}.pdf",
        ...
        "asset_manifest":f"{paper_id}.asset_manifest.json","marker":workspace.marker.name,
    }
    return {
        "schema_version":FORMALIZATION_SCHEMA_VERSION,"paper_number":workspace.paper_number,"paper_id":paper_id,
        ...
        "final_filenames":final_filenames,"marker_rewrite":{"schema_version":"1.0","paper_number":workspace.paper_number,"folder_name":paper_id,"state":"active","planned_paper_id":paper_id},
        "asset_manifest_plan":{"paper_number":workspace.paper_number,"paper_id":paper_id,"asset_names":final_filenames},
...
def write_formalization_plan(workspace: PaperRawWorkspace, *, papers_dir: Path, ledger_path: Path|None=None)->dict:
    ...
    update_status(workspace,"formalization","ready",formalization_sha256=compute_sha256(workspace.formalization),paper_id=plan["paper_id"])
    return plan
...
def assert_formalization_current(workspace: PaperRawWorkspace, *, papers_dir: Path, ledger_path: Path|None=None)->dict:
    ...
    for key in ("paper_number","paper_id","source_workspace","final_directory","source_assets","image_hashes","source_record_hashes","metadata_freeze_sha256","catalog_freeze_sha256","duplicate_inspection_sha256","final_filenames","marker_rewrite","asset_manifest_plan"):
