...
class OriginalLinkResolver(PdfResolver):
    ...
    def _try_direct_pdf(self, doi: str, url: str) -> FetchResult:
        ...
        try:
            ...
            logger.debug("original_link direct PDF failed for {!r}: {}", doi, exc)
            return FetchResult(doi=doi, source=self.name, resolver=self.name,
                               candidate_url=url, error=str(exc))
    ...
    def _try_landing_page(self, doi: str, url: str) -> FetchResult:
        ...
        try:
            ...
            logger.debug("original_link landing page failed for {!r}: {}", doi, exc)
            return FetchResult(doi=doi, source=self.name, resolver=self.name,
                               candidate_url=url, error=str(exc))
        ...
        # If the landing page is itself a PDF, treat it as direct.
        ...
        # Use multi-level landing page resolution for richer HTML parsing.
