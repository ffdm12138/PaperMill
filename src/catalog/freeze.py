...
def _closure(
    ...
    *,
    ...
    errors = []
    catalog_path = folder / f"{paper_number}.catalog.json"
    task_path = folder / f"{paper_number}.catalog_task.json"
    try:
        metadata_freeze = assert_metadata_frozen(
            folder, paper_number,
            asset_prefix=None if require_canonical_directory_name else paper_number,
        )
    except Exception as exc:
        metadata_freeze = {}
        errors.append(f"metadata freeze closure invalid: {exc}")
    try:
        catalog = _load(catalog_path, "catalog")
    except Exception as exc:
        return {}, errors + [str(exc)]
    try:
        task = _load(task_path, "catalog task")
    except Exception as exc:
        return {}, errors + [str(exc)]
    ...
    closure = {
        ...
        "catalog_task_sha256": compute_sha256(task_path),
        "metadata_sha256": hashes.get("metadata_sha256"),
        "metadata_freeze_sha256": hashes.get("metadata_freeze_sha256"),
        "markdown_sha256": hashes.get("markdown_sha256"),
        "conversion_manifest_sha256": hashes.get("conversion_manifest_sha256"),
        "image_hashes": hashes.get("image_hashes") or {},
        "source_record_hashes": hashes.get("source_record_hashes") or {},
        "paper_name": catalog.get("paper_name"),
        "skill_version": task.get("skill_version"),
        "catalog_schema_version": task.get("catalog_schema_version"),
    }
    if metadata_freeze and closure["metadata_sha256"] != metadata_freeze.get("metadata_sha256"):
        errors.append("catalog closure metadata hash mismatch")
...
def freeze_catalog(
    ...
    *,
    ...
    closure, errors = _closure(folder, paper_number, papers_dir=papers_dir,
                               paper_raw_root=paper_raw_root)
    if errors:
        raise ValueError("; ".join(errors))
    receipt = {
        "schema_version": CATALOG_FREEZE_SCHEMA_VERSION,
        "paper_number": paper_number,
        **closure,
        "frozen_at": now_iso(),
    }
    atomic_write_json(folder / f"{paper_number}.catalog_freeze.json", receipt, indent=2)
    return receipt
...
def assert_catalog_frozen(
    ...
    *,
    ...
    receipt = _load(folder / f"{paper_number}.catalog_freeze.json", "catalog freeze receipt")
    errors = []
    ...
    closure, closure_errors = _closure(
        folder, paper_number, papers_dir=papers_dir, paper_raw_root=paper_raw_root,
        require_canonical_directory_name=require_canonical_directory_name,
    )
    errors.extend(closure_errors)
    for key, value in closure.items():
        if receipt.get(key) != value:
            errors.append(f"catalog freeze closure mismatch: {key}")
    if errors:
