...
def normalize_receipt_identity(payload: Mapping[str, Any]) -> dict[str, str]:
    ...
    if paper_number and not PAPER_NUMBER_RE.match(paper_number):
        raise ValueError(
...
def build_receipt_payload(
    *,
    ...
    """
    from src.services.v2_library import now_iso
...
def find_matching_receipt(
    ...
    *,
    ...
    complete staging.
    """
    target = normalize_receipt_identity(
        {
            "candidate_id": candidate_id,
            "page_id": page_id,
            "keyword_id": keyword_id,
    ...
    for root in roots:
        ...
        for path in sorted(Path(root).glob("*/*.discovery_receipt.json")):
            ...
            try:
                ...
                continue
            # Compare only the keys the caller supplied identity for; the
            # receipt on disk may carry a provider the caller does not know.
            if (
